...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'poltergeist':
				dialogue = CoolUtil.coolTextFile(Paths.txt('poltergeist/poltergeistDialogue'));
			case 'transfer':
				dialogue = CoolUtil.coolTextFile(Paths.txt('transfer/transferDialogue'));
			case 'riddle':
				dialogue = CoolUtil.coolTextFile(Paths.txt('riddle/riddleDialogue'));
		}
		switch(SONG.song.toLowerCase())
		{
			case 'poltergeist' | 'paranormal' | 'phantomised':
			{
					defaultCamZoom = 1;
					curStage = 'graveyard';
					var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('graveyard/sky'));
					...
					var clouds:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('graveyard/cloudsmaybe'));
					...
					var hill:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('graveyard/churchhill'));
					hill.setGraphicSize(Std.int(hill.width * 1));
					hill.updateHitbox();
					...
					var stageFront:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('graveyard/front'));
			...
			case 'boopbeepo' | 'shiny' | 'warfare':
			{
					defaultCamZoom = 0.94;
					curStage = 'forest';
					var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('forest/sky'));
					bg.setGraphicSize(Std.int(bg.width * 1));
					bg.antialiasing = true;
					...
					var forestback:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('forest/back2'));
					forestback.setGraphicSize(Std.int(forestback.width * 1));
					forestback.updateHitbox();
					forestback.antialiasing = true;
					forestback.scrollFactor.set(0.7, 0.7);
					forestback.active = false;
					add(forestback);

					var forestback2:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('forest/back1'));
					forestback2.setGraphicSize(Std.int(forestback2.width * 1));
					forestback2.updateHitbox();
					forestback2.antialiasing = true;
					forestback2.scrollFactor.set(0.8, 0.8);
					forestback2.active = false;
					add(forestback2);

					var stageFront:FlxSprite = new FlxSprite(0, -100).loadGraphic(Paths.image('forest/grass'));
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "True" : "Easy") + (Main.watermarks ? " - ANF " + MainMenuState.kadeEngineVer : ""), 16);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			remove(boyfriend);
			boyfriend = new Boyfriend(770, 450, 'bf-sketch');
			add(boyfriend);
			if (iconP1.animation.curAnim.name == 'bf-sketch')
				iconP1.animation.play(SONG.player1);
			else
				iconP1.animation.play('bf-sketch');
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curStep == 800 && curSong == 'Phantomised')
		{
			dad.playAnim('break', true);
		}

		if (curStep >= 832 && curSong == 'Phantomised')
		{
